...
// export default function Left_SmallCards() { 
...
//       const translateX = Math.max(-100, -scrollY * moveIntensityX); // Reversed direction
//       const translateY = Math.min(800, scrollY * moveIntensityY); // Vertical movement remains the same
...
//   return (
//     <div className=" Left_SmallCards_Container"> 
//       <img
...
export default function Left_SmallCards() {
  const [scrollEffect, setScrollEffect] = useState({
    scaleX: 1, // Initial scale is 1 (no scaling)
    scaleY: 1, // Initial scale is 1 (no scaling)
    translateX: 50, // Initial horizontal translation for the first image (more to the right)
    translateY: 0, // Initial vertical translation is 0
  });
  const [isScrolling, setIsScrolling] = useState(false);
  const [scrollTimeout, setScrollTimeout] = useState(null);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const translateX = Math.max(-45, 50 - scrollY * moveIntensityX); // Horizontal movement stops at 0
